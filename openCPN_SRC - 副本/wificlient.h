...
{
    public:
        WIFIWindow(wxFrame *frame, const QString& WiFiServerName);
        ~WIFIWindow();

        void GetSource(QString & source);
...
class WIFIDNSTestThread: public wxThread
{
    public:
        ...
        WIFIDNSTestThread(const QString &name_or_ip);
        ~WIFIDNSTestThread();
        void *Entry();

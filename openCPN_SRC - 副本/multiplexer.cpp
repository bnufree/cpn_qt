...
//            m_pdevmon->StopIOThread(true);
        ...
        {
            ZCHX_LOGMSG(_T("Garmin USB Initialized"));
            ...
            ZCHX_LOGMSG(_T("Sending Routes..."));
            int ret1 = Garmin_GPS_SendRoute(wxString(_T("usb:")), pr, pProgress);

            if(ret1 != 1)
            {
                ZCHX_LOGMSG(_T(" Error Sending Routes"));
                wxString msg;
                msg = _T(" LastGarminError is: ");
                msg += GetLastGarminError();
                ZCHX_LOGMSG(msg);
...
//            m_pdevmon->StopIOThread(true);
        ...
        else
        {
            ...
            ZCHX_LOGMSG(_T("Sending Waypoint..."));
            ...
            if(ret1 != 1)
            {
                ZCHX_LOGMSG(_T(" Error Sending Waypoint to Garmin USB"));
                wxString msg;
                msg = _T(" LastGarminError is: ");
                msg += GetLastGarminError();
                ZCHX_LOGMSG(msg);

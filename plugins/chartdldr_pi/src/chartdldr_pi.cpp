...
{
    ZCHX_LOGMSG(_T("chartdldr_pi: DeInit"));
...
void chartdldr_pi::OnSetupOptions( void )
{
    ...
    if( ! m_pOptionsPage )
    {
        ZCHX_LOGMSG( _T("Error: chartdldr_pi::OnSetupOptions AddOptionsPage failed!") );
        return;
    }
...
//        wxFileName fn(wxStandardPaths::Get().GetDocumentsDir(), wxEmptyString);
        ...
        ZCHX_LOGMSG( _T ( "chartdldr_pi: " ) + m_base_chart_dir );
        ZCHX_LOGMSG( _T ( "chartdldr_pi: " ) + fn.GetPath() );
...
void ChartDldrPanelImpl::UpdateAllCharts( wxCommandEvent& event )
{
    ...
    ZCHX_LOGMSG( wxString::Format(_T("chartdldr_pi::UpdateAllCharts() downloaded %d out of %d charts."), attempted_to_update - failed_to_update, attempted_to_update) );
...
//    ZCHX_LOGMSG(_T("chartdldr_pi:  OCPN_downloadFile done:"));
...
bool ChartDldrGuiAddSourceDlg::LoadSources()
{
    ...
    if( !fn.FileExists() )
    {
        ...
        if( !fn.FileExists() )
        {
            ZCHX_LOGMSG( wxString::Format(_T("Error: chartdldr_pi::LoadSources() %s not found!"), fn.GetFullPath().c_str()) );
            return false;
        }
    }
    wxString path = fn.GetFullPath();

...
void wxMapiSession::Initialise() 
{
    ...
    if (bMapiInstalled)
    {
        ...
        if (m_data->m_hMapi)
        {
            ...
            {
                ZCHX_LOGMSG(_T("MAIL Error: Failed to get one of the functions pointer in MAPI32.DLL\n"));
                Deinitialise();
            }
        }
    }
    else {
        ZCHX_LOGMSG(_("MAIL Error: MAPI32.DLL is not installed or invalid on this computer!"));
    }
}
...
bool wxMapiSession::Logon(const wxString& sProfileName, const wxString& sPassword, wxWindow* pParentWnd)
{
    ...
    if (nError != SUCCESS_SUCCESS && nError != MAPI_E_USER_ABORT)
    {
        ...
        {
            ZCHX_LOGMSG(wxString::Format(_T("MAIL Error: Failed to logon to MAPI using a shared session, Error:%ld\n"), nError));
...
bool wxMapiSession::Logoff()
{
    ...
    if (m_data->m_hSession)
    {
        ...
        if (nError != SUCCESS_SUCCESS)
        {
            ZCHX_LOGMSG(wxString::Format(_T("MAIL Error: Failed in call to MapiLogoff, Error:%ld"), nError));
            m_data->m_nLastError = nError;
            bSuccess = TRUE;
        }
        else
...
bool wxMapiSession::Resolve(const wxString& sName, void* lppRecip1)
{
    ...
    if (nError != SUCCESS_SUCCESS)
    {
        ZCHX_LOGMSG(wxString::Format(_T("MAIL Error: Failed to resolve the name: %s, Error:%ld\n"),
                   sName.c_str(), nError));
        m_data->m_nLastError = nError;
    }
...
bool wxMapiSession::Send(wxMailMessage& message)
{
 ...
 /*   //Setup the "CC" recipients
    ...
    {
        ZCHX_LOGMSG(wxString::Format(_T("MAIL Error: Failed to send mail message, Error:%ld\n"), nError));
        m_data->m_nLastError = nError;
    }

...
{
   QString w_or_e = sentence.Field( east_or_west_field_number );
   Set( sentence.Double( position_field_number ), w_or_e );
}

void LONGITUDE::Set( double position, const QString& east_or_west )
{
//   assert( east_or_west != NULL );

   Longitude = position;
   QString ts = east_or_west;

   if ( !ts.isEmpty() && ts.trimmed()[ 0 ] == 'E' )
   {
      Easting = East;
   }
   else if ( !ts.isEmpty( ) && ts.trimmed()[ 0 ] == 'W' )
   {
      Easting = West;
   }
   else
...
void LONGITUDE::Write( SENTENCE& sentence )
{
    ...
    temp_string.sprintf(("%03d%02d.%03d"), d, m / 1000, m % 1000);

...
wxString GetPluginDataDir(const char* plugin_name)
{
    ...
    ZCHX_LOGMSG(_T("PlugInManager: Using data dirs from: ") + datadirs);
#else
	...
	dir +=
		...
		next = next.Prepend(tryDirName.GetFullPath() + sep);
                ZCHX_LOGMSG(wxString::Format(_T("PlugInManager: using data dir: %s"), next));
                return next;
            }
            more = tryDir.GetNext(&next);
        }
    }
    ZCHX_LOGMSG(_T("WarniÅg: no data directory found, using \"\""));
    return "";
}
...
#endif
    ZCHX_LOGMSG( _T("PlugInManager: plugins loading from ") + dirs);
...
bool PlugInManager::LoadPlugInDirectory(const wxString &plugin_dir, bool load_enabled, bool b_enable_blackdialog)
{
    ...
    for(unsigned int i=0 ; i < file_list.GetCount() ; i++) {
        ...
        {
            ZCHX_LOGMSG(wxString::Format(_T("    %s: %s"), _T("Incompatible plugin detected"), file_name.c_str()));
        ...
        if(pic)
        {
            if(pic->m_pplugin)
            {
                ...
                if(pic->m_bEnabled)
                {
                    ...
                    if(sw.Time() > 10)
                        ZCHX_LOGMSG(_T("PlugInManager: ") + pic->m_common_name
...
bool PlugInManager::CheckPluginCompatibility(wxString plugin_file)
{
    ...
    {
        ZCHX_LOGMSG(wxString::Format(_T("No Import Table! in %s"), plugin_file.c_str()));
    }
    if (virtualpointer)
...
#error No support for other executable formats is implemented.
        ...
        {
            b_compat = false;
            ZCHX_LOGMSG( wxString::Format( _T("    Plugin \"%s\" could not be reliably checked for compatibility."), plugin_file ) );
        }
    }
    else
    {
...
{
	ZCHX_LOGMSG(_T("AddChartToDBInPlace() for file:%s", full_path.c_str));
...
wxScrolledWindow *AddOptionsPage( OptionsParentPI parent, wxString title )
{
    ...
    break;
    default:
        ZCHX_LOGMSG( _T("Error in PluginManager::AddOptionsPage: Unknown parent") );
        return NULL;
    break;
    }

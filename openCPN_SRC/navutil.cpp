...
void MyConfig::LoadNavObjects()
{
    ...
    ZCHX_LOGMSG( _T("Loading navobjects from navobj.xml") );
    ...
    ZCHX_LOGMSG(wxString::Format(_T("Done loading navobjects, %d duplicate waypoints ignored"), wpt_dups) );
    ...
    if( ::wxFileExists( m_sNavObjSetChangesFile ) ) {
        ...
        if(size != 0){
            ZCHX_LOGMSG( _T("Applying NavObjChanges") );
                    ...
                    if(isPersistent)
                    {
                        ...
                        if (!wxDirExists(destf))
                        {
                            if( !wxMkdir(destf, wxS_DIR_DEFAULT) )
                                ZCHX_LOGMSG( _T("Error creating layer directory") );
                        }
...
{
    if ( Switch ){
        ZCHX_LOGMSG( _T("Switch InlandEcdis mode On") );
        LoadS57();
        ...
        ZCHX_LOGMSG( _T("Switch InlandEcdis mode Off") );
...
#define         ONE_DAY_SECS         (24 * 3600)
#define         ONE_HOUR_SEC         3600

QString formatTimeDelta(zchxTimeSpan span)
{
    QString timeStr;
    int days = span.mSecs / ONE_DAY_SECS;
    span -= (ONE_DAY_SECS * days);
    int hours = span.mSecs;
...
double fromDMM( wxString sdms )
{
    ...
    wxString replhelper;
    replhelper = wxString::FromUTF8( "Â´Â·" ); //UKHO PDFs
    sdms.Replace( replhelper, _T(".") );
    replhelper = wxString::FromUTF8( "\"Â·" ); //Don't know if used, but to make sure
    sdms.Replace( replhelper, _T(".") );
    replhelper = wxString::FromUTF8( "Â·" );
    sdms.Replace( replhelper, _T(".") );

...
class Multiplexer : public wxEvtHandler
{
    public:
        ...
        DataStream *FindStream(const QString & port);
        ...
        void SendNMEAMessage(const QString &msg);
        void SetAISHandler(wxEvtHandler *handler);
        void SetGPSHandler(wxEvtHandler *handler);

        int SendRouteToGPS(Route *pr, const QString &com_name, bool bsend_waypoints, wxGauge *pProgress);
        int SendWaypointToGPS(RoutePoint *prp, const QString &com_name, wxGauge *pProgress);

        void OnEvtStream(OCPN_DataStreamEvent& event);
        
        void LogOutputMessage(const QString &msg, QString stream_name, bool b_filter);
        void LogOutputMessageColor(const QString &msg, const QString & stream_name, const QString & color);
        void LogInputMessage(const QString &msg, const QString & stream_name, bool b_filter, bool b_error = false);

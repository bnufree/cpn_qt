...
{
    ZCHX_LOGMSG(_T("Called stub gethostbyaddr_r()"));
    return 0;
}
#endif
...
{
    //  Open a port
    ZCHX_LOGMSG( wxString::Format(_T("Opening NMEA Datastream %s"), m_portstring.c_str()) );
    ...
    //  As a quick check, verify that the specified port is available
            ...
            if(hSerialComm == INVALID_HANDLE_VALUE) {
                ...
                ZCHX_LOGMSG( _T("   Error, comm port open failure, INVALID_HANDLE_VALUE, Datastream skipped.") );
                return;
            }
            else
...
{
    ZCHX_LOGMSG( wxString::Format(_T("Closing NMEA Datastream %s"), m_portstring.c_str()) );
...
//    Kill off the Secondary RX Thread if alive
    if(m_pSecondary_Thread)
    {
        ...
        {
            ZCHX_LOGMSG(_T("Stopping Secondary Thread"));
...
void DataStream::OnSocketReadWatchdogTimer(wxTimerEvent& event)
{
    ...
    if( m_dog_value <= 0 ) {            // No receive in n seconds, assume connection lost
        ZCHX_LOGMSG( wxString::Format(_T("    TCP Datastream watchdog timeout: %s"), m_portstring.c_str()) );
    ...
    switch(event.GetSocketEvent())
    {
        ...
        case wxSOCKET_LOST:
        {
            ...
            if(m_net_protocol == TCP || m_net_protocol == GPSD) {
				if (m_brx_connect_event)
					ZCHX_LOGMSG(wxString::Format(_T("Datastream connection lost: %s"), m_portstring.c_str()));
                if (m_socket_server) {
                    m_sock->Destroy();
                    m_sock=0;
                    break;
        ...
        case wxSOCKET_CONNECTION :
        {
            if(m_net_protocol == GPSD) {
                ...
                ZCHX_LOGMSG( wxString::Format(_T("TCP Datastream connection established: %s"), m_portstring.c_str()) );
                m_dog_value = N_DOG_TIMEOUT;                // feed the dog
                if (m_io_select != DS_TYPE_OUTPUT)
...
GarminProtocolHandler::GarminProtocolHandler(DataStream *parent, wxEvtHandler *MessageTarget, bool bsel_usb)
{
    ...
    if(m_busb) {
        ...
        ZCHX_LOGMSG(_T("Searching for Garmin DeviceInterface and Device..."));
    
        if(!FindGarminDeviceInterface()) {
            ZCHX_LOGMSG(_T("   Find:Is the Garmin USB driver installed?"));
        }
        else {
            if(!ResetGarminUSBDriver())
                ZCHX_LOGMSG(_T("   Reset:Is the Garmin USB Device plugged in?"));
        }
    }
#endif
...
void GarminProtocolHandler::StopSerialThread(void)
{
    ...
    {
        ZCHX_LOGMSG(_T("Stopping Garmin Serial thread"));
        m_Thread_run_flag = 0;
...
void GarminProtocolHandler::StopIOThread(bool b_pause)
{
    ...
    {
        ZCHX_LOGMSG(_T("Stopping Garmin USB thread"));
        m_Thread_run_flag = 0;
...
{
    ZCHX_LOGMSG(_T("Restarting Garmin I/O thread"));
...
bool GarminProtocolHandler::ResetGarminUSBDriver()
{
    ...
    if(GetVersionEx(&version_info))
    {
        if(version_info.dwMajorVersion > 5)
        {
            if(!IsUserAdmin())
            {
                ZCHX_LOGMSG(_T("    GarminUSBDriver Reset skipped, requires elevated privileges on Vista and later...."));
                return true;
            }
        }
    }
    ...
    {
        ZCHX_LOGMSG(_T("   GarminUSBDriver Reset0 failed..."));
        return false;
    }
    ...
    {
        ZCHX_LOGMSG(_T("   GarminUSBDriver Reset1 failed..."));
        return false;
    }
    
    if(!SetupDiCallClassInstaller(DIF_PROPERTYCHANGE,devs,&devInfo))
    {
        ZCHX_LOGMSG(_T("   GarminUSBDriver Reset2 failed..."));
        return false;
    }
    
    ZCHX_LOGMSG(_T("GarminUSBDriver Reset succeeded."));
...
HANDLE GarminProtocolHandler::garmin_usb_start()
{
    ...
    ZCHX_LOGMSG(_T("Garmin USB Device Found"));
    ...
    if((m_usb_handle == INVALID_HANDLE_VALUE) || (m_usb_handle == 0))
    {
        ...
        if (!SetupDiGetDeviceInterfaceDetail(hdevinfo, &infodata,
            pdd, size, NULL, &devinfo))
        {
            ZCHX_LOGMSG(_T("   SetupDiGetDeviceInterfaceDetail failed for Garmin Device..."));
            free(pdd);
            return INVALID_HANDLE_VALUE;
        }
...
*/        
    ...
    if(!DeviceIoControl(m_usb_handle, IOCTL_GARMIN_USB_BULK_OUT_PACKET_SIZE,
        ...
        &size, NULL))
    {
        ZCHX_LOGMSG(_T("   Couldn't get Garmin USB packet size."));
        CloseHandle(m_usb_handle);
        m_usb_handle = INVALID_HANDLE_VALUE;
        return INVALID_HANDLE_VALUE;
    }
    ...
    for(i = 0; i < 25; i++) {
        ...
        if(gusb_cmd_send((const garmin_usb_packet *) oinit, sizeof(oinit)))
        {
                ...
                (le_read32(iresp.gusb_pkt.datasz) == 4))
            {
                ...
                ZCHX_LOGMSG(_T("Successful Garmin USB syncup."));
                return true;;
            }
        }
    }
    ZCHX_LOGMSG(_T("   Unable to establish Garmin USB syncup."));
    return false;
}
...
int GarminProtocolHandler::gusb_cmd_send(const garmin_usb_packet *opkt, size_t sz)
{
    ...
    if (sz && !(sz % m_max_tx_size)) {
        ZCHX_LOGMSG(_T("win_send_call1"));
        gusb_win_send(opkt, 0);
        ZCHX_LOGMSG(_T("win_send_ret1"));
    }
                ...
                case DBT_DEVICEREMOVECOMPLETE:
                    ...
                    switch (pDBHdr->dbch_devicetype)
                        case DBT_DEVTYP_HANDLE:
                            ...
                            ZCHX_LOGMSG(_T("Garmin USB Device Removed"));
                            StopIOThread(false);
                            m_bneed_int_reset = true;
                            processed = true;
                            break;

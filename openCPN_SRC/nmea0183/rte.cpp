...
//   double this_message_number = sentence.Double( 2 );
   ...
   {
      Waypoints.append( sentence.Field( field_number ) ) ;
      field_number++;
   }
...
//            sentence += ("C");             // uppercase required for GPS MLR FFX312
//            sentence += ("c");             // trying lowercase for generic NMEA device
            sentence += QString((QChar)m_complete_char);
            break;
...
//         sentence += "";
   ...
   for(unsigned int i=0 ; i < Waypoints.count() ; i++)
         sentence += Waypoints[i];

   if(m_skip_checksum){
       QString temp_string;
       temp_string.sprintf(("%c%c"), CARRIAGE_RETURN, LINE_FEED );
...
bool RTE::AddWaypoint(const QString& name)
{

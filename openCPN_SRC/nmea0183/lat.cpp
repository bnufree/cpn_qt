...
{
   QString n_or_s = sentence.Field( north_or_south_field_number );
   Set( sentence.Double( position_field_number ), n_or_s );
}

void LATITUDE::Set( double position, const QString& north_or_south )
{
//   assert( north_or_south != NULL );

   Latitude = position;
   QString ts = north_or_south;

   if ( !ts.isEmpty() && ts.trimmed()[ 0 ] == ('N') )
   {
      Northing = North;
   }
   else if (( !ts.isEmpty() ) && ts.trimmed()[ 0 ] == ('S') )
   {
      Northing = South;
   }
   else
...
void LATITUDE::Write( SENTENCE& sentence )
{
   ...
   QString temp_string;
    ...
    temp_string.sprintf(("%02d%02d.%03d"), d, m / 1000, m % 1000);

...
public:
    ...
    bool Open( const QString &senc_file_name );
    void Close();
...
public:
    ...
    bool Open(const QString& ofileName);
...
public:
    ...
    QString getLastError(){ return errorMessage; }
    void setVerbose(bool verbose );
    void setNoErrDialog( bool val ){ m_NoErrDialog = val; }
    
    int ingestHeader(const QString &senc_file_name);
    int ingest(const QString &senc_file_name,
               S57ObjVector *pObjectVector,
               VE_ElementVector *pVEArray,
               VC_ElementVector *pVCArray);
    
    int ingest200(const QString &senc_file_name,
               S57ObjVector *pObjectVector,
               VE_ElementVector *pVEArray,
               VC_ElementVector *pVCArray);
    ...
    QString getSENCFileCreateDate(){ return m_readFileCreateDate; }

    int getSencReadVersion(){ return m_senc_file_read_version; }
    QString getSENCReadBaseEdition(){ return m_read_base_edtn; }
    int getSENCReadLastUpdate(){ return m_read_last_applied_update; }
    int getSENCReadScale(){ return m_Chart_Scale; }
    QString getReadName(){ return m_Name; }
    QString getReadID(){ return m_ID; }
    ...
    int createSenc200(const QString& FullPath000, const QString& SENCFileName, bool b_showProg = true);
    ...
    int GetBaseFileInfo(const QString& FullPath000, const QString& SENCFileName);

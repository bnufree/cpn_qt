...
public:
    ...
    OCP_DataStreamInput_Thread(DataStream *Launcher,
                                  ...
                                  const QString& strBaudRate,
                                  dsPortType io_select
                              );

    ~OCP_DataStreamInput_Thread(void);
    void *Entry();
    bool SetOutMsg(const QString &msg);
    void OnExit(void);
...
private:
    ...
    void ThreadMessage(const QString &msg);
    bool OpenComPortPhysical(const QString &com_name, int baud_rate);
    void CloseComPortPhysical();
    void Parse_And_Send_Posn(const char *s);
    size_t WriteComPortPhysical(char *msg);
    size_t WriteComPortPhysical(const QString& string);
#else
    void ThreadMessage(const QString &msg);
    void Parse_And_Send_Posn(const char *s);
    int OpenComPortPhysical(const QString &com_name, int baud_rate);
    int CloseComPortPhysical(int fd);
    int WriteComPortPhysical(int port_descriptor, const QString& string);

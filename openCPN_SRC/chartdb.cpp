...
{
	ZCHX_LOGMSG(wxString::Format("nEntry = %d, CurrentStackEntry = %d", nEntry, CurrentStackEntry));
      if(nEntry /*&& b_valid*/)
...
{
	ZCHX_LOGMSG(wxString::Format("nEntry = %d, CurrentStackEntry = %d, current_db_index = %d", nEntry, CurrentStackEntry, current_db_index));
...
{
	ZCHX_LOGMSG(wxString::Format("nEntry = %d, stack_index = %d, max_stack = %d", nEntry, stack_index, MAXSTACK));
...
{
	ZCHX_LOGMSG(wxString::Format("nEntry = %d, stack_index = %d, db_index = %d", nEntry, stack_index, db_index));
      if((stack_index >= 0) && (stack_index < nEntry) && (stack_index < MAXSTACK))
...
void ChartDB::DeleteCacheEntry(CacheEntry *pce, bool bDelTexture, const wxString &msg)
{
     ...
     if (msg != wxEmptyString) {
         ZCHX_LOGMSG(wxString::Format(_T("%s%s"), msg.c_str(), ch->GetFullPath().c_str()));
     }
...
{
//    Empty the cache
      ZCHX_LOGMSG(_T("Chart cache purge"));
...
{
    //    Empty the cache
    ZCHX_LOGMSG(_T("Chart cache PlugIn purge"));
...
ChartBase *ChartDB::GetChart(const wxChar *theFilePath, ChartClassDescriptor &chart_desc) const
{
      ...
      if(!fn.FileExists()) {
            ...
            if( !wxDir::Exists(theFilePath) ) {
                  ZCHX_LOGMSG(wxString::Format(wxT("   ...file does not exist: %s"), theFilePath));
                  return NULL;
            }
      }
      ChartBase *pch = NULL;
...
CacheEntry *ChartDB::FindOldestDeleteCandidate( bool blog)
{
    CacheEntry *pret = 0;
        ...
        {
            if(blog)
                ZCHX_LOGMSG(_T("Searching chart cache for oldest entry"));
            int LRUTime = m_ticks;
            int iOldest = 0;
            for(unsigned int i=0 ; i<nCache ; i++)
            {
            ...
            if( !pce->n_lock &&  !isSingleChart(pDeleteCandidate)){
                if(blog)
                    ZCHX_LOGMSG(wxString::Format(_T("Oldest unlocked cache index is %d, delta t is %d"), iOldest, dt));
                
                pret = pce;
            }
            else
                ZCHX_LOGMSG(wxString::Format(_T("All chart in cache locked, size: %d"), nCache));
...
//          ZCHX_LOGMSG(msg);
    ...
    //      Limit cache to n charts, tossing out the oldest when space is needed
      ...
      if(!bInCache)                    // not in cache
      {
            ZCHX_LOGMSG(_T("Creating new chart"));
...
//                chart_class_name = cte.GetChartClassName();
                  ...
                  Ch = NULL;
                  ZCHX_LOGMSG(_T("Unknown chart type"));
            }
            ...
            if(Ch)
            {
                  ...
                  {
                        ZCHX_LOGMSG(wxString::Format(_T("Initializing Chart %s"), msg_fn.c_str()));
                  ...
                  {
                        ZCHX_LOGMSG(wxString::Format(_T("   No PLIB, Skipping vector chart  %s"), msg_fn.c_str()));
...
//                              printf("    Adding chart %d\n", dbindex);
                  ...
                  {
                        ZCHX_LOGMSG(wxString::Format(_T("Problem initializing Chart %s"), msg_fn.c_str()));
...
//          Mark this chart in the database, so that it will not be seen during this run, but will stay in the database
                  ...
                  {
                        ZCHX_LOGMSG(wxString::Format(_T("Recoverable problem initializing Chart %s"), msg_fn.c_str()));
                        delete Ch;
                        Ch = NULL;
                  }
                  ...
                  if(INIT_OK != ir)
                  {
                        ...
                        {
                              ZCHX_LOGMSG(wxString::Format(_T("   OpenChartFromStack... Error opening chart %s ... return code %d"), msg_fn.c_str(), ir));
                        }
                  }

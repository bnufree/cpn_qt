...
public:
      ...
      QString GetFullPath(ChartStack *ps, int stackindex);
      ...
      bool SearchForChartDir(const QString &dir);
      ChartBase *OpenStackChartConditional(ChartStack *ps, int start_index, bool bLargest, ChartTypeEnum Newype, ChartFamilyEnum New_Family_Fallback);

      wxArrayPtrVoid *GetChartCache(void) { return pChartCache; }
      std::vector<int> GetCSArray(ChartStack *ps);

      int GetStackEntry(ChartStack *ps, QString fp);
      bool IsChartInCache(int dbindex);
      bool IsChartInCache(QString path);
...
private:
      ...
      int SearchDirAndAddSENC(QString& dir, bool bshow_prog, bool bupdate);
      bool CreateS57SENCChartTableEntry(QString full_name, ChartTableEntry *pEntry, Extent *pext);

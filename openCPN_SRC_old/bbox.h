...
public:
    ...
    void Expand(const QPointF& , const QPointF&);
    void Expand(const QPointF&);
    ...
    bool LineIntersect(const QPointF& begin, const QPointF& end ) const;
    bool PointInBox( const QPointF&, double Marge = 0) const;
...
#ifdef WX_
    void MapBbox( const QTransform& matrix);
#endif
    ...
    void    SetBoundingBox(const QPointF& a_point);

    void    SetMin(double, double);
    void    SetMax(double, double);
    inline  QPointF GetMin() const;
    inline  QPointF GetMax() const;

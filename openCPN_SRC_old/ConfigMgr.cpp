...
void ConfigMgr::Init()
{
    ...
    if(!wxFileExists(m_configCatalogName)){
        ...
        ZCHX_LOGMSG( _T("Creating new Configs catalog: ") + m_configCatalogName );
...
{
    ZCHX_LOGMSG( _T("Loading Configs catalog: ") + m_configCatalogName );
...
wxString ConfigMgr::CreateNamedConfig( const wxString &title, const wxString &description, wxString UUID )
{
    ...
    if(UUID.IsEmpty()){
        ...
        if(!SaveTemplate( templateFullFileName )){
            ZCHX_LOGMSG(_T("Unable to save template titled: ") + title + _T(" as file: ") + templateFullFileName);
            delete pConfig;
            return _T("");
        }
    }
    ...
    if(!m_configCatalog->AddConfig( pConfig, 0 )){
        ZCHX_LOGMSG(_T("Unable to add config to catalog...Title: ") + title);
        delete pConfig;
        return _T("");
    }
...
#define CHECK_INT(s, t)         read_int = *t; \
                                if(!conf->Read( s , &read_int)) ZCHX_LOGMSG(s); \
